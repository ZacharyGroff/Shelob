...
func TestPutSuccess(t *testing.T) {
	config := config.Config{SeedBuffer: 1}	
...
func TestPutError(t *testing.T) {
	config := config.Config{SeedBuffer: 0}	
	q := NewSeedQueue(&config)
	url, _ := url.Parse("test.com/")
	err := q.Put(*url)
	if err == nil {
...
func TestFlushSize(t *testing.T) {
	...
	config := config.Config{SeedPath: testPath, SeedBuffer: 1, FlushToFile: false}
	q := NewSeedQueue(&config)
	url, _ := url.Parse("test.com/")
...
func TestFlushToFileSuccess(t *testing.T) {
	...
	config := config.Config{SeedPath: testPath, SeedBuffer: 1, FlushToFile: true}
	q := NewSeedQueue(&config)
	url, _ := url.Parse("test.com/")
...
func TestFlushWithoutFileSuccess(t *testing.T) {
	config := config.Config{SeedBuffer: 1, FlushToFile: false}
	q := NewSeedQueue(&config)
	url, _ := url.Parse("test.com/")
